...
| 3.10 | **Scheduler konteyneri hech qachon ishlamagan** | ✅ **tuzatildi** 2026-08-02. `main()` `AsyncIOScheduler.start()` ni event loop tashqarisida chaqirardi → `RuntimeError: no running event loop`, konteyner cheksiz qayta ishga tushardi. Ya'ni 11-bosqichdagi **avtomatik yangilanishning birorta vazifasi bajarilmagan**. Xato faqat konteyner ko'tarilganda ko'rinadi, shuning uchun `tests/test_scheduler_process.py` endi jarayonni haqiqatan ishga tushirib tekshiradi |
